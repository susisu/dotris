...
export class Game extends EventEmitter2 {
  ...
  _redrawHoldedBlock() {
    ...
    if (this._holdedBlockType !== undefined) {
      const block    = Block[this._holdedBlockType];
      const rect     = getClippedBlockRect(block);
      const position = new Point(
        Math.floor((ASIDE_BLOCK_WIDTH - rect.width) / 2),
        Math.floor((ASIDE_BLOCK_HEIGHT - rect.height) / 2)
      );
      const color = this._colors[BlockColorPrefix.BLOCK + this._holdedBlockType];
      for (let j = rect.x; j < rect.x + rect.width; j++) {
        for (let i = rect.y; i < rect.y + rect.height; i++) {
          if (block[i][j] & BLOCK_EXISTS !== 0) {
  ...
  _blockHitTest() {
    for (let j = 0; j < this._blockWidth; j++) {
      for (let i = 0; i < this._blockHeight; i++) {
        if (this._block[i][j] & BLOCK_EXISTS !== 0) {
          const alpha = this._bmp.getAlpha(Point.add(this._blockPosition, new Point(j, i)));
          if (alpha > 0) {
            return true;
          }
        }
  ...
  _willLand() {
    for (let j = 0; j < this._blockWidth; j++) {
      for (let i = this._blockHeight - 1; i >= 0; i--) {
        if (this._block[i][j] & BLOCK_EXISTS !== 0) {
          const alpha = this._bmp.getAlpha(Point.add(this._blockPosition, new Point(j, i + 1)));
          if (alpha > 0) {
            return true;
          }
          break;
  ...
  _getLandPosition() {
    const bottom = new Array(this._blockWidth).fill(-1);
    for (let j = 0; j < this._blockWidth; j++) {
      for (let i = this._blockHeight - 1; i >= 0; i--) {
        if (this._block[i][j] & BLOCK_EXISTS !== 0) {
          bottom[j] = i;
  ...
  _shiftNextBlockType() {
    const blockType = this._nextBlockTypes.shift();
  ...
  _setCurrentBlock(blockType) {
    ...
    if (this._gameMode === GameMode.AUTO) {
      const rect = getClippedBlockRect(this._block);
  ...
  _hold() {
    if (this._holdable) {
      ...
      if (this._holdedBlockType === undefined) {
        ...
        const blockType = this._holdedBlockType;
        this._holdedBlockType = this._blockType;
  ...
  _moveBlock(vector) {
    this._eraseBlockGhost();
    const wouldLand   = this._willLand();
    const oldPosition = this._blockPosition;
  ...
  _rotateBlock(direction) {
    ...
    let succeeded = false;
    if (this._blockHitTest()) {
      const corrs = BlockCorrection[this._blockType][this._blockRotationDegree][direction];
    ...
    if (succeeded) {
      const directionPM = direction === BlockRotationDirection.CLOCKWISE ? 1 : -1;
  ...
  _landBlock() {
    ...
    for (let j = 0; j < this._blockWidth; j++) {
      for (let i = 0; i < this._blockHeight; i++) {
        if (this._block[i][j] & BLOCK_EXISTS !== 0) {
          this._lineCounter[this._blockPosition.y - BORDER_THICKNESS + i] += 1;
        }
      }
    }
    this._deleteLines();
  ...
  _deleteLines() {
    const spin     = this._blockSpinTest();
    const spinMini = this._blockSpinMiniTest();
    let minI = 0;
    ...
    if (deleteCounter > 0) {
      ...
      const b2bText = this._backToBack ? "Back-To-Back " : "";
