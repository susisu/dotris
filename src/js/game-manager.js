...
export class GameManager extends EventEmitter2 {
  constructor() {
    ...
    this._onGameOver = () => {
      this._over = true;
      const resultText = `Size: ${this._width} x ${this._height}, `
  ...
  _rescale(full) {
    if (this._scaling) {
      const scale = this._width / this._height >= window.innerWidth / window.innerHeight
      ...
      if (this._highResolution) {
        const scale = 1.0 / (window.devicePixelRatio || 1.0);
        this._wrapper.style.transform = `scale(${scale},${scale})`;
      }
      else {
        this._wrapper.style.transform = "none";
  ...
  start(config) {
    if (!this._game) {
      ...
      const colors = ColorScheme[config.colorScheme] || ColorScheme.standard;
